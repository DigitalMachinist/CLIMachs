var Shell = Shell || {
  ...
  registerCommand: function ( cmd, sig, desc, fn ) {
    ...
    if ( Shell.commands[ cmd ] ) {
      if ( ( Shell.commands[ cmd ].signature != sig ) || ( Shell.commands[ cmd ].description != desc ) || ( Shell.commands[
          cmd ].callback != fn ) ) {
  ...
  permissionCommand: function ( args, msg ) {
    ...
    if ( playerId ) {
      var players = _.union( findObjs( {
        _type: "player",
        _displayname: playerId
      } ), findObjs( {
        _type: "player",
        _d20userid: playerId
      } ) );
      ...
      if ( players.length != 1 ) {
        Shell.write( "Please try again using one of: " + ( _.map( players, function ( p ) {
          return p.get( '_d20userid' );
        } ) ).join( ", " ), msg.who );
        return;
      }
    ...
    case "add":
      ...
      if ( _.contains( state.Shell.userPermissions[ cmd ], playerId ) ) {
        return;
      }
      state.Shell.userPermissions[ cmd ].splice( _.sortedIndex( state.Shell.userPermissions[ cmd ], playerId ), 0,
        playerId );
      break;
    case "remove":
  ...
  hasPermission: function ( msg, cmd ) {
    if ( Shell.isFromGM( msg ) ) {
      return true;
    }
    if ( state.Shell.userPermissions[ cmd ] ) {
      if ( _.contains( state.Shell.userPermissions[ cmd ], msg.playerid, true ) ) {
        return true;
      }
      if ( _.contains( state.Shell.userPermissions[ cmd ], "", true ) ) {
        return true;
      }
    }
    /////
