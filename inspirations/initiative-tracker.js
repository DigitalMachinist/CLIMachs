var Tracker = Tracker || {
  ...
  handleTurnChange: function ( newTurnOrder, oldTurnOrder ) {
    var newTurns = JSON.parse( ( typeof ( newTurnOrder ) == typeof ( "" ) ? newTurnOrder : newTurnOrder.get(
      'turnorder' ) || "[]" ) );
  ...
  setConfigParam: function ( who, param, value ) {
    ...
    for ( var i = 0; i < Tracker.CONFIG_PARAMS.length; i++ ) {
      if ( Tracker.CONFIG_PARAMS[ i ][ 0 ] == param ) {
        state.InitiativeTracker[ Tracker.CONFIG_PARAMS[ i ][ 0 ] ] = ( value == null ? !state.InitiativeTracker[
          Tracker.CONFIG_PARAMS[ i ][ 0 ] ] : value );
        err = false;
        break;
      }
    }
  ...
  showTrackerHelp: function ( who, cmd ) {
    ...
    helpMsg +=
      "get [PARAM]:        display the value of the specified config parameter, or all config parameters\n";
  ...
  handleTrackerMessage: function ( tokens, msg ) {
    ...
    case "set":
      if ( tokens.length <= 2 ) {
        Tracker.write( "Error: The 'set' command requires at least one argument (the parameter to set)", who, "",
          "Tracker" );
        break;
      }
      var value = true;
    ...
    case "enable":
      if ( tokens.length != 3 ) {
        Tracker.write( "Error: The 'enable' command requires exactly one argument (the parameter to enable)", who,
          "", "Tracker" );
        break;
      }
    ...
    case "disable":
      if ( tokens.length != 3 ) {
        Tracker.write( "Error: The 'disable' command requires exactly one argument (the parameter to disble)", who,
          "", "Tracker" );
        break;
      }
    ...
    case "toggle":
      if ( tokens.length != 3 ) {
        Tracker.write( "Error: The 'toggle' command requires exactly one argument (the parameter to toggle)", who,
          "", "Tracker" );
        break;
      }
  ...
  addStatus: function ( tokenId, duration, status, name ) {
    ...
    state.InitiativeTracker.status.push( {
      'token': tokenId,
  ...
  showStatusHelp: function ( who, cmd ) {
    ...
    helpMsg += "help:               display this help message\n";
    helpMsg +=
      "add DUR ICON DESC:  add DUR rounds of status effect with specified icon and description to selected tokens\n";
  ...
  handleStatusMessage: function ( tokens, msg ) {
    ...
    case "add":
      ...
      if ( tokens.length < 5 ) {
        Tracker.write( "Error: The 'add' command requires three arguments (duration, icon, description)", who, "",
          "Tracker" );
        break;
      }
    ...
    case "show":
      if ( ( !selected ) || ( selected.length <= 0 ) ) {
        Tracker.write( "Error: The '" + tokens[ 1 ] + "' command requires at least one selected token", who, "",
          "Tracker" );
        break;
      }
      var tokenIds = [];
    ...
    case "del":
      if ( ( tokens.length == 2 ) && ( selected ) && ( selected.length > 0 ) ) {
        ...
        for ( var i = 0; i < state.InitiativeTracker.status.length; i++ ) {
          ...
          for ( var j = 0; j < selected.length; j++ ) {
            if ( ( selected[ j ]._type != "graphic" ) || ( selected[ j ]._id != status.token ) ) {
              continue;
            }
      ...
      if ( tokens.length != 3 ) {
        Tracker.write( "Error: The '" + tokens[ 1 ] + "' command requires an argument (status effect ID)", who, "",
          "Tracker" );
        break;
      }
  ...
  registerTracker: function () {
    ...
    if ( ( typeof ( Shell ) != "undefined" ) && ( Shell ) && ( Shell.registerCommand ) ) {
      Shell.registerCommand( "!tracker", "!tracker <subcommand> [args]", "Configure the initiative tracker",
        Tracker.handleTrackerMessage );
